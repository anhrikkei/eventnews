...
def index(request, baiviet_id):
    ...
    ds_baiviet_danhmuc = Baiviet.objects.filter(danh_muc_id=baiviets.danh_muc_id, trang_thai='on').order_by('ma_bai')[::-1]
    ds_tintop = Baiviet.objects.filter(trang_thai='on').order_by('luot_xem')[::-1][0:4]
